...
function DashboardPage() {
  ...
  }, [quotations, receivables, bankLines, openAudits]);
    ...
    <div className="min-w-0 space-y-4">
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-5">
        ...
        <Card className="min-w-0 lg:col-span-2">
          <CardHeader>
            <CardTitle>Việc cần duyệt (từ dữ liệu hệ thống)</CardTitle>
          </CardHeader>
          <CardBody className="space-y-2">
            {!approvals.length ? (
              <div className="rounded-[var(--radius-md)] border border-border-soft px-3 py-6 text-center text-sm text-muted">
                Không có việc chờ — dữ liệu trống hoặc đã xử lý hết.
              </div>
            ) : (
              approvals.map((a) => (
                <Link
