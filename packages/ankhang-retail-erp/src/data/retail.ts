...
export type StoreConfig = {
  ...
  vietQrStyle: "compact2" | "print" | "qr_only";
  qrAddInfoPrefix: string;
  paymentQrMode: "vietqr" | "upload";
  customPaymentQrDataUrl: string;
  customPaymentQrNote: string;
  customPaymentQrUpdatedAt: string;
  speakerEnabled: boolean;
  speakerProvider: SpeakerProvider;
  speakerDeviceId: string;
  speakerApiUrl: string;
  speakerWebhookSecret: string;
  speakerPushAmount: boolean;
  defaultPayMethod: PayMethod;
  openShiftDefaultCash: number;
  allowManualConfirmQr: boolean;
  edcEnabled: boolean;
  edcTerminalId: string;
  autoConfirmQr: boolean;
  confirmTimeoutSec: number;
  soundOnScan: boolean;
  autoFocusScan: boolean;
};

export type PayMethod = "cash" | "qr" | "card" | "points";
export type QrTemplate = "vietqr" | "text" | "custom_url";
export type SpeakerProvider =
  | "generic"
  | "momo"
  | "vnpay"
  | "zalopay"
...
export type PendingPayment = {
  ...
  content: string;
  method: "qr" | "card";
  status: "waiting" | "confirmed" | "cancelled" | "paid";
  createdAt: string;
  paidAt?: string;
  provider?: string;
  externalId?: string;
...
export const SETUP_STEPS = [
  { id: "welcome", title: "Tổng quan A → Z", short: "Lộ trình", hint: "Lộ trình master data → POS → BI" },
  { id: "store", title: "Hồ sơ cửa hàng", short: "Cửa hàng", hint: "Tên, MST, ngân hàng, QR thanh toán" },
  { id: "products", title: "Danh mục hàng", short: "Sản phẩm", hint: "SKU, barcode, UOM, giá" },
  { id: "suppliers", title: "Nhà cung cấp", short: "NCC", hint: "Lead time, on-time rate" },
  { id: "customers", title: "Khách hàng CRM", short: "Khách", hint: "Hạng, điểm, lịch mua" },
  { id: "inbound", title: "Nhập kho GRN", short: "Nhập", hint: "PO → GRN → MAP → lô/HSD" },
  { id: "pos", title: "POS & thanh toán", short: "POS", hint: "Quét mã, promo, QR/cash" },
  { id: "inventory", title: "Kiểm kho", short: "Kiểm kho", hint: "Cycle count, variance" },
  { id: "done", title: "Hoàn tất", short: "Xong", hint: "Vận hành & làm sạch demo" },
] as const;

export const VN_BANKS: { bin: string; name: string; short: string }[] = [
...
function storeDefaults(): StoreConfig {
  return {
    storeName: "",
    ...
    speakerProvider: "generic",
    speakerDeviceId: "",
    speakerApiUrl: "",
    speakerWebhookSecret: "",
    speakerPushAmount: true,
    defaultPayMethod: "cash",
    openShiftDefaultCash: 2_000_000,
    allowManualConfirmQr: true,
    edcEnabled: false,
    edcTerminalId: "",
    autoConfirmQr: false,
...
export function normalizeStoreConfig(
  partial: Partial<StoreConfig> & Pick<StoreConfig, "storeName" | "productName">,
): StoreConfig {
  const base = { ...storeDefaults(), ...partial };
  return {
    ...base,
    storeName: base.storeName.trim(),
    productName: base.productName.trim(),
    taxId: base.taxId.trim(),
    address: base.address.trim(),
    phone: base.phone.trim(),
    bankAccount: base.bankAccount.trim(),
    bankName: base.bankName.trim(),
    bankBin: base.bankBin.trim(),
    accountName: base.accountName.trim(),
    currency: base.currency || "VND",
    vatDefault: Number(base.vatDefault) || 8,
    qrTemplate: base.qrTemplate || "vietqr",
    vietQrStyle: base.vietQrStyle || "compact2",
    qrAddInfoPrefix: base.qrAddInfoPrefix || "AK",
    paymentQrMode: base.paymentQrMode || "vietqr",
    customPaymentQrDataUrl: base.customPaymentQrDataUrl || "",
    customPaymentQrNote: base.customPaymentQrNote || "",
    customPaymentQrUpdatedAt: base.customPaymentQrUpdatedAt || "",
    speakerEnabled: Boolean(base.speakerEnabled),
    speakerProvider: base.speakerProvider || "generic",
    speakerDeviceId: base.speakerDeviceId || "",
    speakerApiUrl: base.speakerApiUrl || "",
    speakerWebhookSecret: base.speakerWebhookSecret || "",
    speakerPushAmount: Boolean(base.speakerPushAmount),
    defaultPayMethod: base.defaultPayMethod || "cash",
    openShiftDefaultCash: Number(base.openShiftDefaultCash) || 0,
    allowManualConfirmQr: Boolean(base.allowManualConfirmQr),
    edcEnabled: Boolean(base.edcEnabled),
    edcTerminalId: base.edcTerminalId || "",
    autoConfirmQr: Boolean(base.autoConfirmQr),
    confirmTimeoutSec: Number(base.confirmTimeoutSec) || 120,
    soundOnScan: Boolean(base.soundOnScan),
    autoFocusScan: Boolean(base.autoFocusScan),
  };
}

export function allFlagsTrue(): SetupFlags {
  return {
    store: true,
    products: true,
    suppliers: true,
...
export function setupCompletion(flags: SetupFlags): number {
  const keys = Object.keys(EMPTY_SETUP_FLAGS) as (keyof SetupFlags)[];
...
export function tierLabel(tier: Customer["tier"]): string {
  return TIER_LABEL[tier];
}

export function buildPaymentQr(input: {
  store: StoreConfig;
  amount: number;
  content: string;
}): {
  ...
  const { store, amount, content } = input;
  if (store.paymentQrMode === "upload" && store.customPaymentQrDataUrl) {
    return {
      imageUrl: store.customPaymentQrDataUrl,
      content,
      authenticityNote: "QR tĩnh do cửa hàng upload — kiểm tra số tiền trước khi chuyển.",
      amount: Math.max(0, Math.round(amount)),
      mode: "upload",
    };
  }
  const bin = store.bankBin || "970436";
  const acc = (store.bankAccount || "").replace(/\D/g, "");
  const style = store.vietQrStyle || "compact2";
  const name = encodeURIComponent(store.accountName || store.storeName || "");
  const addInfo = encodeURIComponent(content || "");
  const amt = Math.max(0, Math.round(amount));
  const imageUrl =
    acc
      ? `https://img.vietqr.io/image/${bin}-${acc}-${style}.png?amount=${amt}&addInfo=${addInfo}&accountName=${name}`
      : "";
  return {
    imageUrl,
    content,
    authenticityNote: "VietQR động — số tiền và nội dung gắn trong mã.",
    amount: amt,
    mode: "vietqr",
  };
}

export const SEED_STORE: StoreConfig = normalizeStoreConfig({
  ...
  bankBin: "970436",
  accountName: "SIEU THI AN KHANG",
  currency: "VND",
  vatDefault: 8,
});

export const BLANK_STORE: StoreConfig = normalizeStoreConfig({
  storeName: "",
  productName: "AnKhang Retail ERP",
  taxId: "",
  address: "",
